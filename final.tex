...
Filter, welcher als ein Schätzer für die zukünftige Wahrscheinlichkeitsverteilung
...
Drei Fehlerarten
Entfernungsfehler: Fehler beim Zurücklegen einer geraden Strecke
Drehfehler: Fehler beim Drehen des Roboters
Driftfehler: Orientierungsfehler beim Zurücklegen einer geraden Strecke
Bewegungsmodell in ARIA/ARNL bildet genau diese Fehler ab.
\appendix
\section{Appendix I}

\paragraph{Map}                      Map of the environment we'll use to navigate
...
\paragraph{KDegPerDeg} 0.05           minimum 0,  When the robot rotates, the error in the
...
\paragraph{FuseAllSensors} false      ARNL uses a Kalman filter which allows you to
...
\paragraph{ReflectorVariance} 10000   minimum 0,  This number will be used as the variance
                          of the (x, y) coords of the center of the reflectors
                          in the R matrix of the Kalman filter.

\paragraph{Qxx} 100                   minimum 0,  This is the first element of the
                          diagonal covariance matrix which will define the
                          error in the kalman model for the X axis.

\paragraph{Qyy} 100                   minimum 0,  This is the second element of the
                          diagonal covariance matrix which will define the
                          error in the kalman model for the Y axis.

\paragraph{Qtt} 1                     minimum 0,  This is the third element of the
                          diagonal covariance matrix which will define the
                          error in the kalman model for the Theta axis.

\paragraph{ReflectorMatchDist} 1000   minimum 0,  When finding the closest reflector in
                          the map to an observed reflection, this is the
                          maximum distance the system will search to find the
                          closest reflector.

\paragraph{ReflectorMaxRange} 32000   minimum 0,  This is the maximum distance that the
                          SICK lrf is capable of seeing a reflector. (This is
                          smaller than the max range of the regular SICK
                          readings)

\paragraph{ReflectorMaxAngle} 45      minimum 0,  This is the maximum angle of incidence
...
\paragraph{UseReflectorCenters} true  The Kalman filter matches the returns from the laser
...
\paragraph{Triangulate} false         The regular Kalman filter tries to fuse the encoder
...
\paragraph{TriangulateScoreThreshold} 0.5  When the map of the environment is very close
...
\paragraph{AdjustNumSamplesFlag} false  The number of samples is by default kept high to
...
\paragraph{MinNumSamples} 200         minumum 0,  When the AdjustSamplesFlag is set to
...
\paragraph{NumSamplesAngleFactor} 1   minimum 0,  When the AdjustSamplesFlag is set to
                          true the number of samples is reduced as the
...
\begin{thebibliography}{------}
	...
	\bibitem[Whitbrook1]{whitbrook1}
		...
		http://www.ais.fraunhofer.de/~delipetk
\end{thebibliography}
\end{document}